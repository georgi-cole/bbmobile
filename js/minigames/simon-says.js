        ...
        if(currentIndex === length){
          ...
          if(g.GameUtils && g.GameUtils.evaluateOutcome && !debugMode && competitionMode){
            ...
            if(rawScore >= 60 && !outcome.didWin && !g.cfg?.debugAlwaysWin){
              ...
              console.log('[SimonSays] Win probability applied: success forced to loss, score:', finalScore);
            }
            
            if(outcome.didWin){
              console.log('[SimonSays] Player won! Reasons:', outcome.reasons.join('; '));
            } else {
              console.log('[SimonSays] Player lost. Reasons:', outcome.reasons.join('; '));
            }
          }
