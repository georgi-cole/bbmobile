(function(g){
  ...
  UI.INJECTED_CSS = [
    ...
    '  .settingsTabPane[data-pane="cast"] .cast-strip{padding:8px;width:100%;box-sizing:border-box}',
    '  .settingsTabPane[data-pane="cast"] .cast-form{gap:6px}',
    '  .settingsTabPane[data-pane="cast"] .cast-form label{font-size:.75rem}',
    '  .settingsTabPane[data-pane="cast"] .cast-form input, .settingsTabPane[data-pane="cast"] .cast-form select{font-size:14px;padding:8px 10px}',
    '}',
  ...
  function wireLaunchMinigameButton(modal){
    ...
    btn.addEventListener('click', () => {
      ...
      if(g.CompetitionFlow && typeof g.CompetitionFlow.launchFullscreenMinigame === 'function'){
        ...
        if(unlimited){
          console.info('[ui.config-and-settings] Using unlimited debug timer (∞)');
          options.unlimited = true;
          options.timeLimit = null;
          options.disablePhaseTimerSync = true;
        } else {
          console.info('[ui.config-and-settings] Using configured timer:', configDuration, 'seconds');
