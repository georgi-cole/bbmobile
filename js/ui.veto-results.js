...
(function(global){
  ...
  /**
   * Render inline POV winner card (simplified version without scores)
   * Shows winner's name and avatar with shield badge
   * @param {number} winnerId - ID of the POV winner
   * @param {Object} options - Configuration options
   * @param {number} options.displayDurationMs - How long to display (default: 3000ms)
   * @param {Function} options.onDismiss - Callback when card is dismissed
   * @param {Array<string>} options.ffwdSelectors - Selectors for fast-forward close
   * @returns {HTMLElement} The created inline winner card element
   */
  function renderInlinePOVWinner(winnerId, options){
    options = options || {};
    var displayDurationMs = typeof options.displayDurationMs === 'number' ? options.displayDurationMs : 3000;
    var onDismiss = typeof options.onDismiss === 'function' ? options.onDismiss : null;
    var ffwdSelectors = options.ffwdSelectors || null;

    // Get player info
    var info = getPlayerInfo(winnerId);
    
    // Clean up any existing inline winner cards
    try{ 
      var old = document.querySelectorAll('.pov-inline-winner'); 
      old && old.forEach(function(n){ 
        try{ 
          if(n.__vetoAutoDismissTimer) clearTimeout(n.__vetoAutoDismissTimer); 
        }catch(e){} 
    ...
    // Create container
    const container = document.createElement('div');
    container.className = 'pov-inline-winner comp-results';
    container.setAttribute('role','region');
    container.setAttribute('aria-label','POV Winner');

    // Header
    const header = document.createElement('div');
    header.className = 'comp-results-header';
    header.innerHTML = `<h3>POV Winner</h3>`;
    container.appendChild(header);

    // Winner tile (simplified - no rank or score display)
    const tile = document.createElement('div');
    tile.className = 'comp-player-tile first-place pov-winner-tile';
    tile.setAttribute('role','group');
    tile.setAttribute('aria-label', `${info.name} won the Power of Veto`);

    // Avatar with onerror fallback
    const avatarContent = info.avatarHtml 
      ? info.avatarHtml 
      : (info.avatarUrl 
          ? `<img src="${info.avatarUrl}" alt="${info.name}" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';"><div class="avatar-fallback" style="display:none;"></div>`
          : '<div class="avatar-fallback" style="display:flex;"></div>');

    // Simplified tile layout: avatar + name + shield (no rank or score)
    tile.innerHTML = `
      <div class="comp-avatar">${avatarContent}</div>
      <div class="comp-meta">
        <div class="comp-name">${info.name}</div>
      </div>
      <div class="comp-badge" aria-hidden="true">🛡️</div>
    `;
    ...
    // Attach fast-forward close handler
    attachFastForwardClose(container, ffwdSelectors);
    ...
    console.info('[veto-results] Inline POV winner card displayed for', info.name);
    return container;
  }

  global.VetoResultsUI = global.VetoResultsUI || {};
  global.VetoResultsUI.renderVetoCompResults = renderVetoCompResults;
  global.VetoResultsUI.renderInlinePOVWinner = renderInlinePOVWinner;
